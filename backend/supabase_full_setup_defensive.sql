-- supabase_full_setup_defensive.sql
-- 여러 번 재실행해도 안전하도록 DROP IF EXISTS를 늘리고 defensive checks를 추가한 버전

-- DROP existing policies/triggers/functions/tables if necessary (주의: 데이터 삭제 가능)
-- 이 섹션은 운영 DB에서는 신중하게 사용하세요.

-- 안전을 위해 기존 트리거/함수/정책을 삭제
DROP TRIGGER IF EXISTS set_timestamp ON admins;
DROP TRIGGER IF EXISTS set_timestamp_stores ON stores;
DROP TRIGGER IF EXISTS set_timestamp_menus ON menus;
DROP TRIGGER IF EXISTS set_timestamp_markets ON markets;
DROP TRIGGER IF EXISTS set_timestamp_orders ON orders;
DROP TRIGGER IF EXISTS set_timestamp_site_visits ON site_visits;

DROP FUNCTION IF EXISTS trigger_set_timestamp() CASCADE;
DROP FUNCTION IF EXISTS is_admin() CASCADE;

-- 테이블은 DROP 하지 않음(필요하면 주석 해제하여 사용)

-- 1) 테이블 생성 (IF NOT EXISTS로 안전하게 생성)
...
-- 5) RLS 방어적 설정 (DROP IF EXISTS 포함)
